...
const MatchManagement = ({ match, goBack }) => {
  ...
  const handlePlayerAction = async (playerId, action) => {
    setPlayerStats((prev) => {
      const copy = { ...prev, [playerId]: { ...(prev[playerId] || {}), goals: 0, fouls: 0, assists: 0 } };
  ...
  const getPlayerObj = (entry) => {
    ...
    const id = typeof entry === "string" ? entry : entry?._id ? String(entry._id) : null;
  ...
  <div className="flex flex-col h-[70%] bg-gray-100">
        ...
        <div className="bg-white shadow-gray-300 rounded-lg shadow-md p-4 ">
          ...
          <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
            {(match.teamA || []).map((p) => {
              ...
              const id = player?._id ? String(player._id) : (typeof p === "string" ? p : "");
              const stats = playerStats[id] || { goals: 0, fouls: 0, assists: 0 };
              return (
                <div key={id} className="flex items-center gap-3 hover:bg-gray-50 p-2 rounded transition">
                  <img
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-gray-800">{player?.name || id}</div>
                    <div className="text-sm text-gray-500">{player?.category || "—"}</div>
                    <div className="text-xs text-gray-600 mt-1">
                      Goals: {stats.goals} | Fouls: {stats.fouls} | Assists: {stats.assists}
                    </div>
                  </div>
    ...
    <div className="bg-gradient-to-r   shadow-md shadow-blue-300 from-blue-600 to-purple-600 text-white shadow-inner p-4 border-t sticky bottom-0 z-10">
      <div className="flex justify-between text-sm font-medium ">
        ...
        <div>
          <strong>{match.teamBName}</strong> — Score: {teamTotals.teamBScore} | Goals: {teamTotals.teamBGoals} | Fouls: {teamTotals.teamBFouls}
        </div>
      </div>
    </div>
  </div>
